...
X = np.load("X.npy") #(1035, 34716)
X = np.nan_to_num(X)
Y = np.load("Y.npy") #(1035,)
...
for i in range(len(num_hidden_l1)):
    for j in range(len(num_hidden_l2)):
        for k in range(len(num_hidden_l3)):
            ...
            callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=early_stopping_count)]# checkpoint]
            #Fit the model
            resu = model.fit(trainX, trainY, batch_size=batch_size, epochs=epochs, validation_data=(testX,testY), verbose=1, callbacks=callbacks)
            ...
            result[3].append(resu.history['loss'][-1*early_stopping_count-1])
            result[4].append(resu.history['categorical_accuracy'][-1*early_stopping_count-1])
            result[5].append(resu.history['val_loss'][-1*early_stopping_count-1])
            result[6].append(resu.history['val_categorical_accuracy'][-1*early_stopping_count-1])
            result[7].append(len(resu.history['loss'])-early_stopping_count)
            print("train loss:%.10g, train accuracy:%.10g, test loss:%.10g, test accuracy:%.10g for layers:%d-%d-%d at epoch %d"
                  %(resu.history['loss'][-1*early_stopping_count-1], resu.history['categorical_accuracy'][-1*early_stopping_count-1], resu.history['val_loss'][-1*early_stopping_count-1],
                  resu.history['val_categorical_accuracy'][-1*early_stopping_count-1],num_hidden_l1[i], num_hidden_l2[j], num_hidden_l3[k], len(resu.history['loss'])-early_stopping_count))
...
result.to_csv('LR_0.0005_R_0.001_SGD_4.csv')
